...
public class TestFlowTests
{
    ...
    public void Can_Complete_TestFlow_Dialog_With_Unhealthy_Advice()
    {
        ...
        dialog = sut.Continue(dialog); // Welcome -> How old are you?
        dialog.CurrentPartId.Value.Should().Be("Age");
        dialog = sut.Continue(dialog); // How old are you -> empty answer -> validation error
        dialog.CurrentPartId.Value.Should().Be("Age");
        ...
        dialog = sut.Continue(dialog); // Sports types -> empty answer -> unhealthy
        dialog.CurrentPartId.Value.Should().Be("Unhealthy");
        dialog = sut.Continue(dialog); // Unhealthy -> e-mail address
        dialog.CurrentPartId.Value.Should().Be("Email");
        dialog = sut.Continue(dialog); // E-mail address -> empty answer -> completed
