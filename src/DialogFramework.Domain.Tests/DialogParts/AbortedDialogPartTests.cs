...
public class AbortedDialogPartTests
{
    [Fact]
    public void Can_Convert_Entity_To_Builder()
    {
        // Arrange
        var input = (IDialogPart)new AbortedDialogPartBuilder()
    ...
    {
        // Arrange
        var input = (IDialogPartBuilder)new AbortedDialogPartBuilder()
            .WithId(new DialogPartIdentifierBuilder().WithValue("Test"));
        ...
        // Assert
        actual.Should().BeEquivalentTo(((AbortedDialogPartBuilder)input).Build());
    }
}
